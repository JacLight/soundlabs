...
const staticPaths = ['_next', 'assets', 'favicon.ico', 'robots.txt', 'sitemap.xml', 'site.webmanifest', 'tt'];

//implementation of lodash round function;
...
export const isStaticPath = (slug: string[]) => {
  if (slug.length === 0) return false;
  console.log('isStaticPath ->', slug);
  return staticPaths.some((path) => {
    return slug[0].startsWith(path);
  });
};
...
/**
 * Format a date
 * @param date The date to format (Date object or ISO date string)
